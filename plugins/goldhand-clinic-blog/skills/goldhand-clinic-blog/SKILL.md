...
- 플러그인 소유자가 이 스킬·스크립트·자산을 수정한 작업에서는 전체 검증 뒤 플러그인 루트의 `scripts/refresh_plugin.py`를 실행한다. 소유자 전용 `publisher.json`이 자동 배포를 켠 환경에서는 이 명령이 완전한 원본 동기화, GitHub 푸시, Windows CI 성공 확인, 새 Release와 두 공개 자산의 SHA-256 검증, 공개 설치 테스트까지 끝내야 완료다. CI나 공개 자산 검증이 실패하면 배포 완료라고 보고하지 않는다. 사용자 설치 환경에는 소유자 GitHub 권한이나 배포 설정을 만들거나 요구하지 않는다.
