# 내부 평문 검수 뒤 네이버 제작 계약

이 문서는 내부 평문 검수가 끝난 직후 자동으로 적용한다. 글 구조와 문장을 만드는 문서가 아니다. 구조의 유일한 기준은 `information-delivery-structure.md`다.

## 절대 보존값

- 내부 검수를 마친 제목과 평문은 띄어쓰기와 조사까지 고치지 않는다.
...
- 제작 중 문장 수정이 필요하면 내부 평문 단계로 돌아가 구조와 생활어를 다시 검수한 뒤 사용자에게 묻지 않고 제작을 계속한다.

## HTML 역할

내부 최종 평문을 `<article data-goldhand-type="정보전달형">` 하나로 조립한다.
...
- 모바일 줄바꿈은 내부 최종 문장의 단어와 순서를 바꾸지 않고 시각적 줄만 나눈다.
- 강조는 읽는 데 실제로 도움이 되는 짧은 구절에만 사용하며, 개수를 맞추려고 문장을 고치지 않는다.
- 외부 업체의 클래스, 로고, 사진, 색상, 지도, 연락처를 복사하지 않는다.

## 이미지

- 이미지는 내부 평문 검수가 끝나면 자동으로 계획하고 생성한다. 이미지 진행 여부를 사용자에게 묻지 않는다.
...
- 이미지 생성이나 게시가 실패하면 내부 최종 평문을 유지한 채 질문 없이 이미지 없는 결과로 전환한다.
...
검사기는 내부 최종 문장을 고치는 도구가 아니다. 구조나 HTML 문제가 발견되면 조립을 고치고, 문장 자체의 수정이 필요하면 내부 평문 검수 단계로 돌아갔다가 자동 제작을 재개한다.
