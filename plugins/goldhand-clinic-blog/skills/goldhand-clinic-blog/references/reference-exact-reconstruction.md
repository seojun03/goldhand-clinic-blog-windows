...
본문까지 검토한 정보글 한 편을 제목 심리·도입 설득·정보 공개 흐름·전환·미세 표현 기능·마무리 감정의 **편집 마스터**로 고정한다. 일반 정보는 주제에 맞는 저장 레퍼런스 여러 편과 필요한 네이버 보충 자료에서 별도로 모으되, 편집 마스터의 기존 글 구조는 바꾸지 않는다. 모든 출처 문장은 복사하지 않고 금손 공식 `goldhand-official-voice-v1`로 새로 쓰며, 꾸밈은 `goldhand-naver-native-v4`로 고정한다.

## 핵심 정의

`레퍼런스 방식처럼 쓴다`는 말은 제목만 비슷하게 만드는 것이 아니다. 선택한 편집 마스터 한 편의 다음 구조·표현 기능을 같은 순서로 재현한다.
...
다음은 편집 마스터와 모든 일반 정보 출처에서 전부 폐기한다. 금손 관련 내용이 필요하면 `clinic-facts.md`의 사실로만 새로 쓴다.
...
어느 출처든 특정 한의원의 의학 주장, 경력·성과 수치는 근거가 아니다. 완성 원고의 업체 정보·원장 경험·사례는 `clinic-facts.md`만 사용한다. 의료 내용은 `mergedInformationAtoms`의 일반 정보이며, 변동 가능하거나 고위험인 내용은 필요한 한국 공식 의료 자료로 확인한다. 제목·도입·흐름·표현 기능은 단일 `reference-writing-intelligence.json` 프로필에서 해석하고, 문장 호흡과 종결어미는 그 기능을 지우지 않는 범위에서 금손 공식 말투로 자연화한다.
...
편집 마스터 ID나 URL이 둘 이상 섞이면 발행하지 않는다. 기존 레이아웃 호환을 위한 `data-content-reference-source`도 편집 마스터 URL을 유지한다. 실제 다중 일반 정보 출처는 article 구조에 추가하지 않고 내부 `content-sources.json`에서만 관리한다. 다른 레퍼런스의 구조·색상·카드를 가져오는 것도 금지한다.
...
1. 일반 정보 선택기의 `mergedInformationAtoms`와 `content-sources.json`, 편집 마스터의 `referenceWritingIntelligence`, `approvedWritingLessons`를 분리해 읽는다. 어느 출처의 완성 문장도 분석·복사 거리 대조 외의 초안 재료로 쓰지 않는다.
...
6. 각 `mergedInformationAtoms[].id`와 편집 마스터의 `flowBeats[].id`에 일반 정보, 필요한 금손 사실, 환자 장면, 다음 문단으로 넘어갈 이유를 대응시킨다. 숫자 약속 제목은 같은 개수의 실제 답을 기존 본문 구조 안에 번호로 둔다.
...
8. 매 절마다 `원인-설명-예외-자가관리`를 기계적으로 반복하지 않는다. 선택한 편집 마스터의 진행 방식과 전환 논리를 따른다.
...
8. 이미지 위치는 기존 금손 구조와 편집 마스터의 논리적 슬롯을 따른다. 이미지 자체는 금손 공식 블로그의 `safeAuto: true` 자산 또는 사용자 제공 파일만 사용한다.
...
- `reader-question`은 확정 주제·제목의 대표 독자 고민을 편집 마스터의 질문 기능으로 쓴 2~3개, `solution-preview`는 정확히 1개이며 마지막 질문 뒤·본문 전에 와야 한다.
...
- 모든 일반 정보 출처와 편집 마스터 업체의 상호·인명·지역·연락처·경력·성과·프로그램·사례·사진 URL이 없어야 한다. `validate_information_sources.py`의 동적 차단 엔터티 검사도 통과해야 한다.
